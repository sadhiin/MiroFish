"""
Simulated Configuration Intelligent Generator
...
Adopt a step-by-step generation strategy to avoid failure due to generating overly long content at once:
...
CHINA_TIMEZONE_CONFIG = {
    ...
    "activity_multipliers": {
        ...
        "night": 0.5       # Decrease late at night
    }
}
...
class AgentActivityConfig:
    ...
    # Active hours (24-hour system, 0-23)
...
class TimeSimulationConfig:
    """Time simulation configuration (based on Chinese work and rest habits)"""
    # Total simulation duration (simulation hours)
    total_simulation_hours: int = 72  # Default simulation of 72 hours (3 days)
    
    # Time represented by each round (simulation minutes) - default 60 minutes (1 hour), accelerate time flow
    minutes_per_round: int = 60
    
    # Range of the number of Agents activated per hour
    agents_per_hour_min: int = 5
    agents_per_hour_max: int = 20
    
    # Peak period (19-22 in the evening, the most active time for Chinese people)
    peak_hours: List[int] = field(default_factory=lambda: [19, 20, 21, 22])
    peak_activity_multiplier: float = 1.5
    
    # Off-peak period (0-5 in the early morning, almost no activity)
...
class PlatformConfig:
    ...
    # Viral threshold (number of interactions needed to trigger spread)
    viral_threshold: int = 10
    
    # Echo chamber strength (degree of similar viewpoints clustering)
...
class SimulationConfigGenerator:
    ...
    def generate_config(
        ...
        logger.info("Assigning suitable publisher Agent for initial posts...")
        ...
        for attempt in range(max_attempts):
            try:
                response = self.client.chat.completions.create(
                    ...
                    # Do not set max_tokens, let LLM be free
                )
...
- The user group is Chinese, and it needs to conform to Beijing time habits
...
- Peak period from 19-22 PM (activity coefficient 1.5)
- Activity decreases after 23 PM (activity coefficient 0.5)
- General rule: low activity at midnight, increasing in the morning, moderate during work hours, peak in the evening
- **Important**: The following example values are for reference only, you need to adjust specific time periods according to the nature of the event and the characteristics of the participating group
  - For example: the peak for student groups may be 21-23 PM; media is active all day; official institutions only during working hours
  - For example: sudden hotspots may lead to discussions late at night, off_peak_hours can be appropriately shortened
...
- peak_hours (int array): Peak periods, adjusted according to the event participating group
- off_peak_hours (int array): Off-peak periods, usually late at night
...
- reasoning (string): Brief explanation of why this configuration
    ...
    def _get_default_time_config(self, num_entities: int) -> Dict[str, Any]:
        """Get default time configuration (Chinese time habits)"""
        return {
            "total_simulation_hours": 72,
    ...
    def _parse_time_config(self, result: Dict[str, Any], num_entities: int) -> TimeSimulationConfig:
        """Parse time configuration results and verify that agents_per_hour value does not exceed the total number of agents"""
        # Get original values
        agents_per_hour_min = result.get("agents_per_hour_min", max(1, num_entities // 15))
        agents_per_hour_max = result.get("agents_per_hour_max", max(5, num_entities // 5))
        
        # Verify and correct: ensure it does not exceed the total number of agents
...
- Describe the direction of public opinion
- Design initial post content, **each post must specify poster_type (poster type)**

**Important**: poster_type must be selected from the "available entity types" above, so that the initial post can be assigned to the appropriate Agent for posting.
...
{{
    ...
    def _assign_initial_post_agents(
        ...
        Match the most suitable agent_id according to each post's poster_type
        """
        ...
        # Type mapping table (handle different formats that LLM might output)
        type_aliases = {
        ...
        # Record the used agent index for each type to avoid using the same agent repeatedly
    ...
    def _generate_agent_configs_batch(
        ...
        # Construct entity information (using configured summary length)
        entity_list = []
...
- **Time aligns with Chinese daily routine**: Almost no activity from 0-5 AM, most active from 19-22 PM
...
{{
    "agent_configs": [
        ...
        system_prompt = "You are a social media behavior analysis expert. Return pure JSON, configurations must align with Chinese daily routine."
        ...
        # Construct AgentActivityConfig objects
        configs = []
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        """Generate a single Agent configuration based on rules (Chinese lifestyle)"""
        entity_type = (entity.get_entity_type() or "Unknown").lower()
        
        if entity_type in ["university", "governmentagency", "ngo"]:
            # Official institutions: Active during working hours, low frequency, high impact
            return {
                "activity_level": 0.2,
                "posts_per_hour": 0.1,
            ...
            # Media: Active all day, medium frequency, high impact
            return {
                "activity_level": 0.5,
                "posts_per_hour": 0.8,
