...
Convert entities from Zep graph to the Agent Profile format required by the OASIS simulation platform

Optimization improvements:
1. Use Zep retrieval function to enrich node information twice
2. Optimize prompt word generation for very detailed personas
...
class OasisProfileGenerator:
    ...
    Convert entities from the Zep graph to the Agent Profile required for OASIS simulation
    
    Optimization features:
    1. Use Zep graph retrieval function to obtain richer context
    2. Generate very detailed personas (including basic information, professional experience, personality traits, social media behavior, etc.)
    3. Distinguish between individual entities and abstract group entities
    """
    ...
    # Group/institution type entities (need to generate group representative personas)
    GROUP_ENTITY_TYPES = [
    ...
    def generate_profile_from_entity(
        ...
        Args:
            ...
            use_llm: Whether to use LLM to generate detailed profile
        ...
        if use_llm:
            ...
            # Use rules to generate basic profile
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Use Zep graph hybrid search function to obtain rich information related to the entity
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        3. Enriched information retrieved by Zep hybrid search
        """
        context_parts = []
        ...
        # 4. Use Zep hybrid search to obtain more enriched information
        ...
        if zep_results.get("facts"):
            ...
            if new_facts:
                context_parts.append("### Facts Information Retrieved by Zep\n" + "\n".join(f"- {f}" for f in new_facts[:15]))
    ...
    def _is_group_entity(self, entity_type: str) -> bool:
        """Determine if it is a group/institution type entity"""
    ...
    def _generate_profile_with_llm(
        ...
        # Try generating multiple times until successful or reaching the maximum retry count
        max_attempts = 3
        last_error = None
        ...
        for attempt in range(max_attempts):
            try:
                response = self.client.chat.completions.create(
                    ...
                    # Do not set max_tokens, let LLM perform freely
                )
                
                content = response.choices[0].message.content
                
                # Check if it was truncated (finish_reason is not 'stop')
    ...
    def _try_fix_json(self, content: str, entity_name: str, entity_type: str, entity_summary: str = "") -> Dict[str, Any]:
        ...
        if json_match:
            ...
            try:
                ...
                # 5. If it still fails, try more aggressive fixes
                try:
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are a social media user persona generation expert. Generate detailed, realistic personas for public opinion simulation, maximizing the restoration of existing real situations. Must return valid JSON format, all string values must not contain unescaped newlines. Use Chinese."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for the entity, maximizing the restoration of existing real situations.
...
Please generate JSON, including the following fields:
1. bio: Social media profile, 200 characters
2. persona: Detailed persona description (2000 characters of plain text), must include:
   ...
   - Stance and views (attitude towards topics, content that may provoke anger/emotion)
   - Unique features (catchphrases, special experiences, personal hobbies)
   - Personal memories (important part of the persona, introduce the individual's association with events, and the individual's actions and reactions in events)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account setting for the institution/group entity, maximizing the restoration of existing real situations.
...
1. bio: Official account profile, 200 characters, professional and appropriate
2. persona: Detailed account setting description (2000 characters of plain text), must include:
   - Basic information of the institution (official name, nature of the institution, background of establishment, main functions)
   ...
   - Characteristics of published content (content type, publishing frequency, active time periods)
   ...
   - Institutional memory (important part of the institution's persona, introduce the institution's association with events, and the institution's actions and reactions in events)
3. age: Fixed at 30 (virtual age of the institution account)
4. gender: Fixed at "other" (institution account uses "other" to indicate non-individual)
5. mbti: MBTI type, used to describe account style, such as ISTJ representing rigorous and conservative
6. country: Country (e.g., "China")
...
- Use Chinese (except the gender field must be in English "other")
- age must be the integer 30, gender must be the string "other"
- The institution account's speech must conform to its identity positioning"""
    ...
    def generate_profiles_from_entities(
        ...
        Args:
            ...
            parallel_count: Number of parallel generations, default is 5
            realtime_output_path: File path for real-time writing (if provided, writes each time one is generated)
        ...
        def generate_single_profile(idx: int, entity: EntityNode) -> tuple:
            ...
            try:
                ...
                # Real-time output of the generated profile to the console and log
        ...
        print(f"\n{'='*60}")
        print(f"Starting generation of Agent profiles - Total {total} entities, parallel count: {parallel_count}")
        print(f"{'='*60}\n")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=parallel_count) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_entity):
                ...
                try:
                    ...
                    if error:
                        logger.warning(f"[{current}/{total}] {entity.name} used fallback profile: {error}")
                    else:
                    ...
                    # Real-time write to file (even for backup personas)
                    save_profiles_realtime()
        
        print(f"\n{'='*60}")
        print(f"Persona generation completed! A total of {len([p for p in profiles if p])} Agents generated")
        print(f"{'='*60}\n")
        
        return profiles
    
    def _print_generated_profile(self, entity_name: str, entity_type: str, profile: OasisAgentProfile):
        """Real-time output of generated persona to console (full content, not truncated)"""
        separator = "-" * 70
        
        # Construct full output content (not truncated)
    ...
    def save_profiles(
        ...
        """
        Save Profile to file (choose the correct format based on platform)
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        - user_char: Detailed persona description (injected into LLM system prompts, guiding Agent behavior)
        - description: Short public introduction (displayed on user profile page)
        
        Difference between user_char and description:
        - user_char: For internal use, LLM system prompts, determines how the Agent thinks and acts
        - description: External display, visible introduction to other users
        """
        import csv
