...
Interface 2: Use Zep API to Construct Standalone Graph
"""
...
class GraphBuilderService:
    """
    Graph Construction Service
    Responsible for calling Zep API to construct knowledge graph
    """
...
def add_text_batches(
    ...
    def _wait_for_episodes(
        ...
        if progress_callback:
            progress_callback(f"Start waiting for {total_episodes} text blocks to be processed...", 0)
        ...
        while pending_episodes:
            ...
            if progress_callback:
                progress_callback(
                    f"Processing in Zep... {completed_count}/{total_episodes} completed, {len(pending_episodes)} pending ({elapsed} seconds)",
...
# Get edges (pagination)
    ...
    def get_graph_data(self, graph_id: str) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary containing nodes and edges, including detailed data such as time information and attributes
        """
        nodes = fetch_all_nodes(self.client, graph_id)
        edges = fetch_all_edges(self.client, graph_id)
