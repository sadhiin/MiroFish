...
class ZepEntityReader:
    ...
    2. Filter out nodes that match predefined entity types (nodes whose labels are not just Entity)
    3. Get related edge and associated node information for each entity
    """
    ...
    def _call_with_retry(
        ...
        Args:
            ...
            max_retries: Maximum number of retries (default is 3 times, i.e., try up to 3 times)
    ...
    def get_all_nodes(self, graph_id: str) -> List[Dict[str, Any]]:
        ...
        logger.info(f"Retrieved a total of {len(nodes_data)} nodes")
        return nodes_data

    def get_all_edges(self, graph_id: str) -> List[Dict[str, Any]]:
        """
        Get all edges of the graph (paged retrieval)
        ...
        logger.info(f"Retrieved a total of {len(edges_data)} edges")
        return edges_data
    
    def get_node_edges(self, node_uuid: str) -> List[Dict[str, Any]]:
        """
        Get all related edges of the specified node (with retry mechanism)
    ...
    def filter_defined_entities(
        ...
        Filtering logic:
        - If the node's Labels only contain "Entity", it means this entity does not match our predefined types, skip it
        - If the node's Labels contain tags other than "Entity" and "Node", it means it matches the predefined types, keep it
        ...
        logger.info(f"Filtering completed: Total nodes {total_count}, Matching conditions {len(filtered_entities)}, "
    ...
    def get_entity_with_context(
        ...
        """
        Get a single entity and its complete context (edges and related nodes, with retry mechanism)
