...
Implement a simple command/response pattern through the file system:
1. Flask writes commands to the commands/ directory
2. Simulation script polls the command directory, executes the command, and writes the response to the responses/ directory
3. Flask polls the response directory to get the result
"""
    ...
    def send_batch_interview(
        ...
        Args:
            ...
            platform: Default platform (optional, will be overridden by each interview item's platform)
                - "twitter": Default to interview only on Twitter platform
                - "reddit": Default to interview only on Reddit platform
...
class SimulationIPCServer:
    """
    Simulated IPC server (used by simulation script side)
    ...
    def poll_commands(self) -> Optional[IPCCommand]:
        """
        Poll the command directory and return the first command to be processed
