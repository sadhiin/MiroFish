...
Run simulations in the background and record each Agent's actions, supporting real-time status monitoring
"""
...
# Flag to indicate whether cleanup function is registered
...
class SimulationRunState:
    ...
    # Recent actions (for real-time display on the front end)
...
class SimulationRunner:
    ...
    2. Parse the run log and record each Agent's actions
    ...
    def start_simulation(
        ...
        graph_id: str = None  # Zep graph ID (required when enabling graph updates)
    ) -> SimulationRunState:
        """
        Start simulation
        ...
        Args:
            ...
            graph_id: Zep graph ID (required when enabling graph updates)
        ...
        if enable_graph_memory_update:
            if not graph_id:
                raise ValueError("Graph ID must be provided when enabling graph memory update")
        ...
        try:
            ...
            # This can fix issues with third-party libraries (like OASIS) not specifying encoding when reading files
            env = os.environ.copy()
            env['PYTHONUTF8'] = '1'  # Supported in Python 3.7+, makes all open() use UTF-8 by default
            env['PYTHONIOENCODING'] = 'utf-8'  # Ensure stdout/stderr use UTF-8
            
            # Set working directory to simulation directory (databases and other files will be generated here)
            # Use start_new_session=True to create a new process group, ensuring all subprocesses can be terminated via os.killpg
            process = subprocess.Popen(
                ...
                start_new_session=True,  # Create new process group, ensure all related processes can be terminated when server shuts down
            )
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                # Update round information (from round_end event)
                                    ...
                                    # Overall rounds take the maximum value of the two platforms
                                    if round_num > state.current_round:
                                        state.current_round = round_num
                                    # Overall time takes the maximum value of the two platforms
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        Determine if a platform is enabled by checking if the corresponding actions.jsonl file exists
        ...
        # If the platform is enabled but not completed, return False
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        """
        Terminate the process and its subprocesses across platforms
        ...
        if IS_WINDOWS:
            ...
            # /F = Force termination, /T = Terminate process tree (including subprocesses)
            ...
            try:
                ...
                try:
                    ...
                    # Force termination
                    logger.warning(f"Process not responding, force termination: {simulation_id}")
                    subprocess.run(
            ...
            try:
                ...
                # If it still hasn't ended after the timeout, force send SIGKILL
                logger.warning(f"Process group not responding to SIGTERM, force termination: {simulation_id}")
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Get platform: prioritize using the platform in the record, otherwise use the default platform
    ...
    def get_all_actions(
        ...
        # If platform-specific files do not exist, try reading the old single file format
        if not actions:
    ...
    def cleanup_all_simulations(cls):
        ...
        Called when the server shuts down to ensure all subprocesses are terminated
        """
        ...
        # Check if there is content to clean (avoid printing useless logs for empty processes)
        ...
        for simulation_id, process in processes:
            try:
                if process.poll() is None:  # Process is still running
                    ...
                    try:
                        # Use a cross-platform method to terminate the process
    ...
    def register_cleanup(cls):
        ...
        # In Flask debug mode, register cleanup only in the reloader subprocess (the process actually running the application)
        # WERKZEUG_RUN_MAIN=true indicates it's the reloader subprocess
        # If not in debug mode, this environment variable doesn't exist, and registration is still needed
        ...
        if is_debug_mode and not is_reloader_process:
            _cleanup_registered = True  # Mark as registered to prevent subprocesses from trying again
            return
    ...
    def check_env_alive(cls, simulation_id: str) -> bool:
        ...
        Returns:
            True indicates the environment is alive, False indicates the environment is closed
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        if not os.path.exists(sim_dir):
            return False
    ...
    def get_env_status_detail(cls, simulation_id: str) -> Dict[str, Any]:
        ...
        Returns:
            Status detail dictionary, including status, twitter_available, reddit_available, timestamp
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Specify platform (optional)
                ...
                - None: Interview both platforms during dual-platform simulation, return integrated results
    ...
    def interview_agents_batch(
        ...
        Args:
            ...
            platform: Default platform (optional, will be overridden by each interview item's platform)
                - "twitter": Default to interview only the Twitter platform
                - "reddit": Default to interview only the Reddit platform
                - None: Each Agent interviews both platforms simultaneously in dual-platform simulation
            timeout: Timeout (seconds)
    ...
    def close_simulation_env(
        ...
        Send a command to the simulation to close the environment, allowing it to gracefully exit the waiting command mode
    ...
    def get_interview_history(
        ...
        Args:
            ...
            limit: Limit on the number returned per platform
        ...
        # Determine the platform to query
        if platform in ("reddit", "twitter"):
            platforms = [platform]
        else:
            # If platform is not specified, query both platforms
        ...
        # If multiple platforms were queried, limit the total number
