"""Zep Graph paging utility.

Zep's node/edge list endpoint uses UUID cursor for pagination,
This module encapsulates automatic pagination logic (including single-page retry), transparently returning complete lists to callers.
"""
...
def _fetch_page_with_retry(
    ...
    **kwargs: Any,
    ...
    """Single page request, retry with exponential backoff on failure. Only retries transient errors like network/IO."""
    if max_retries < 1:
...
def fetch_all_nodes(
    ...
    """Paginated fetching of graph nodes, up to max_items (default 2000). Each page request has built-in retry."""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """Paginated fetching of all graph edges, returns full list. Each page request has built-in retry."""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
