...
# Adding this prefix can prevent the Agent from calling tools and respond directly with text
INTERVIEW_PROMPT_PREFIX = "Combining your persona, all past memories, and actions, respond to me directly in text without calling any tools:"


def optimize_interview_prompt(prompt: str) -> str:
    """
    Optimize Interview questions, adding a prefix to prevent the Agent from calling tools.
...
def get_graph_entities(graph_id: str):
    ...
    Query Parameters:
        ...
        enrich: Whether to retrieve related relationship information (Default true)
    """
    try:
...
def create_simulation():
    ...
    Note: max_rounds and other parameters are intelligently generated by LLM, no manual setting required.
    ...
    try:
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": "The project has not yet built a graph, please call /api/graph/build first"
            }), 400
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    Note: Run scripts (run_*.py) stay in the backend/scripts/ directory and are no longer copied to the simulation directory.
    ...
    try:
        ...
        # If config_generated=True and files exist, the preparation is considered complete
        # The following statuses indicate preparation complete:
        # - ready: Prepared, can run
        # - preparing: If config_generated=True indicates completed
        # - running: Running, indicates preparation completed long ago
        # - completed: Run completed, indicates preparation completed long ago
        # - stopped: Stopped, indicates preparation completed long ago
        # - failed: Run failed (but preparation completed)
        ...
        if status in prepared_statuses and config_generated:
            ...
            # If status is preparing but file generation is complete, automatically update status to ready
            if status == "preparing":
                try:
            ...
            logger.info(f"Simulation {simulation_id} detection result: Preparation completed (status={status}, config_generated={config_generated})")
            return True, {
                "status": status,
            ...
            logger.warning(f"Simulation {simulation_id} detection result: Preparation not completed (status={status}, config_generated={config_generated})")
            return False, {
...
def prepare_simulation():
    ...
    Features:
    - Automatically detect completed preparation tasks to avoid duplicate generation.
    - If already prepared, return existing results directly.
    - Support forced regeneration (force_regenerate=true).
    
    Steps:
    1. Check if there are existing completed preparation works.
    ...
    try:
            ...
            if is_prepared:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "message": "Already completed preparation work, no need to regenerate",
        ...
        if not simulation_requirement:
            return jsonify({
                "success": False,
                "error": "The project is missing a simulation requirement description (simulation_requirement)"
            }), 400
        ...
        try:
            ...
            reader = ZepEntityReader()
            # Fast read of entities (no edge info needed, just counting)
            filtered_preview = reader.filter_defined_entities(
                graph_id=state.graph_id,
                defined_entity_types=entity_types_list,
                enrich_with_edges=False  # No edge info, speed up
            )
            ...
            # Failure doesn't affect subsequent flow, background task will refetch
        ...
        # Update simulation status (with pre-fetched entity count)
        ...
        def run_prepare():
            try:
                ...
                def progress_callback(stage, progress, message, **kwargs):
                    ...
                    # Construct detailed progress info
                    stage_names = {
...
def get_prepare_status():
    ...
    2. Check if a completed preparation exists via simulation_id.
    ...
    try:
        ...
        # If simulation_id is provided, check if preparation is already completed first
        if simulation_id:
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
            if is_prepared:
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "progress": 100,
                        "message": "Already completed preparation work",
                        "already_prepared": True,
                        "prepare_info": prepare_info
                    }
                })
        ...
        if not task_id:
            if simulation_id:
                # simulation_id exists but not yet prepared
                return jsonify({
                    "success": True,
                    "data": {
                        "simulation_id": simulation_id,
        ...
        if not task:
            # Task doesn't exist, but if simulation_id is provided, check if pre-completed
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def get_simulation(simulation_id: str):
    ...
    try:
        ...
        # If the simulation is ready, attach running instructions
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    """
    Get the latest report_id corresponding to the simulation.
    
    Iterate through the reports directory, find reports with matching simulation_id,
    and return the latest one (sorted by created_at) if there are multiple.
...
def get_simulation_history():
    ...
    Used for homepage history project display, returns simulation list with project name, description, etc.
    
    Query Parameters:
        limit: Returns count limit (default 20)
    ...
    try:
        ...
        # Enriched simulation data, only read from Simulation file
        ...
        for sim in simulations:
            ...
            if run_state:
                ...
                # Use total_rounds set by user, fallback to recommended rounds if not available
            ...
            # Get file list of associated project (max 3)
...
def get_simulation_profiles_realtime(simulation_id: str):
    ...
    Differences from /profiles interface:
    - Reads files directly, doesn't go through SimulationManager
    - Suitable for real-time viewing during generation
    - Returns extra metadata (e.g. file modification time, whether generating, etc.)
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "platform": "reddit",
                "count": 15,
                "total_expected": 93,  # Total expected count (if any)
...
def get_simulation_config_realtime(simulation_id: str):
    ...
    Differences from /config interface:
    - Reads files directly, doesn't go through SimulationManager
    - Suitable for real-time viewing during generation
    - Returns extra metadata (e.g. file modification time, whether generating, etc.)
    - Can return partial info even if config generation is not completed
...
def get_simulation_config(simulation_id: str):
    ...
    Includes:
        ...
        - generation_reasoning: LLM's configuration reasoning description
    """
    try:
        manager = SimulationManager()
        ...
        if not config:
            return jsonify({
                "success": False,
                "error": f"Simulation config does not exist, please call /prepare interface first"
            }), 404
...
def download_simulation_config(simulation_id: str):
    ...
    try:
        ...
        if not os.path.exists(config_path):
            return jsonify({
                "success": False,
                "error": "Config file does not exist, please call /prepare interface first"
            }), 404
...
def download_simulation_script(script_name: str):
    """
    Download simulation script files (General scripts, located in backend/scripts/)
    
    Optional script_name values:
...
def start_simulation():
    ...
    Request (JSON):
        {
            ...
            "max_rounds": 100,                     # Optional: max rounds, used to truncate long simulations
            "enable_graph_memory_update": false,   # Optional: whether to update Agent activities to Zep memory
            "force": false                         # Optional: force restart (stops running simulation and cleans logs)
        }
 
    Regarding force parameter:
        - If enabled and the simulation is running or completed, it will be stopped first and logs cleaned.
        - Cleaned content include: run_state.json, actions.jsonl, simulation.log, etc.
        - Won't clean config files (simulation_config.json) or profile files.
        - Suitable for re-running simulation scenarios.
 
    Regarding enable_graph_memory_update:
        - If enabled, all Agent activities (posting, commenting, liking, etc.) in simulation will be updated to Zep graph in real-time.
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "graph_memory_update_enabled": true,  # Whether memory update is enabled
    ...
    try:
        ...
        # Smart state handling: allow restart if preparation is completed
        if state.status != SimulationStatus.READY:
            # Check if preparation is completed
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)

            if is_prepared:
                # Preparation is completed, check if there are running processes
                if state.status == SimulationStatus.RUNNING:
                    # Check if the simulation process is really running
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            try:
                                ...
                                logger.warning(f"Warning during simulation stop: {str(e)}")
                        else:
                            return jsonify({
                                "success": False,
                ...
                # If in force mode, clear running logs
                if force:
                    logger.info(f"Force mode: Clear simulation logs {simulation_id}")
                    cleanup_result = SimulationRunner.cleanup_simulation_logs(simulation_id)
                    if not cleanup_result.get("success"):
                        logger.warning(f"Warning occurred while clearing logs: {cleanup_result.get('errors')}")
...
def get_run_status_detail(simulation_id: str):
    """
    Get detailed simulation running status (including all actions)
    ...
    try:
        ...
        # recent_actions only shows two platforms' content of the latest round
...
def get_simulation_actions(simulation_id: str):
    ...
    Query Parameters:
        limit: Returns count (default 100)
...
def get_simulation_posts(simulation_id: str):
    ...
    Query Parameters:
        ...
        limit: Returns count (default 50)
        offset: Offset
...
def interview_agent():
    ...
    Note: This feature requires the simulation environment to be in a running state (entering wait-for-command mode after finishing the simulation loop).

    Request (JSON):
        {
            "simulation_id": "sim_xxxx",       # Required, simulation ID
            "agent_id": 0,                     # Required, Agent ID
            "prompt": "What's your opinion?",  # Required, interview question
            "platform": "twitter",             # Optional, specify platform (twitter/reddit)
                                               # When not specified: simultaneous interview on both platforms for dual-platform simulations
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "Simulation environment is not running or has been closed. Please ensure simulation is completed and entered command waiting mode."
            }), 400
        ...
        return jsonify({
            "success": False,
            "error": f"Wait for Interview response timeout: {str(e)}"
        }), 504
...
def interview_agents_batch():
    ...
    Note: This feature requires simulation environment to be in a running state.
    ...
    Request (JSON):
        {
            ...
            "interviews": [                    # Required, interview list
                ...
                {
                    "agent_id": 1,
                    "prompt": "What's your opinion on B?"  # Uses default platform value if not specified
                }
            ],
            "platform": "reddit",              # Optional, default platform (overridden by per-item platform)
                                               # When not specified: each Agent simultaneous interview on both platforms for dual-platform simulations
    ...
    try:
        ...
        for i, interview in enumerate(interviews):
            if 'agent_id' not in interview:
                return jsonify({
                    "success": False,
                    "error": f"Item {i+1} in interview list missing agent_id"
                }), 400
            if 'prompt' not in interview:
                return jsonify({
                    "success": False,
                    "error": f"Item {i+1} in interview list missing prompt"
                }), 400
            ...
            if item_platform and item_platform not in ("twitter", "reddit"):
                return jsonify({
                    "success": False,
                    "error": f"platform for item {i+1} in interview list can only be 'twitter' or 'reddit'"
                }), 400
        ...
        # Optimize prompt for each interview item, add prefix to prevent Agent from calling tools
        ...
        return jsonify({
            "success": False,
            "error": f"Wait for batch Interview response timeout: {str(e)}"
        }), 504
...
def interview_all_agents():
    ...
    Request (JSON):
        {
            "simulation_id": "sim_xxxx",            # Required, simulation ID
            "prompt": "What's your overall opinion on this?",  # Required, interview question (all Agents use same question)
    ...
    try:
        ...
        return jsonify({
            "success": False,
            "error": f"Wait for global Interview response timeout: {str(e)}"
        }), 504
...
def close_simulation_env():
    ...
    Send close environment command to simulation, allowing it to exit wait-for-command mode gracefully.
    
    Note: This is different from /stop interface, /stop will force termination of process,
    while this interface lets simulation gracefully close environment and exit.
