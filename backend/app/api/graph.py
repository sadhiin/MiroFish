...
Uses the project context mechanism, persisting state on the server.
"""
...
def build_graph():
    ...
    try:
        ...
        if project.status == ProjectStatus.GRAPH_BUILDING and not force:
            return jsonify({
                "success": False,
                "error": "Graph is building, please do not resubmit. To force rebuild, add force: true",
        ...
        def build_task():
            ...
            try:
                ...
                # Wait for Zep processing (query processed status of each episode)
                task_manager.update_task(
                    task_id,
                    message="Waiting for Zep data processing...",
                    progress=55
                )
        ...
        return jsonify({
            ...
            "data": {
                "project_id": project_id,
                "task_id": task_id,
                "message": "Graph building task started, please query progress via /task/{task_id}"
            }
        })
